-- Enforce offer expiry inside accept_offer_atomic RPC.
-- Previously the function only checked status = 'approved'.
-- A parent could call the RPC directly (bypassing UI guards) and accept an expired offer.
-- This migration adds an explicit expiry check at the database level.
...
BEGIN
  ...
  -- Reject if the offer window has closed
  ...
  -- Withdraw all other active applications for the same child
  UPDATE public.applications
