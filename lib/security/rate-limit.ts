...
function toRedisKey(scope: string, key: string) {
  return `rl:${scope}:${key}`
}
...
function toRetryAfterSec(resetAtMs: number, currentTime: number) {
  return Math.max(1, Math.ceil((resetAtMs - currentTime) / 1000))
}

async function enforceInMemoryRateLimit(input: RateLimitInput): Promise<RateLimitResult> {
...
function parseUpstashPipelineResult(payload: unknown) {
...
async function enforceUpstashRateLimit(input: RateLimitInput): Promise<RateLimitResult | null> {
  ...
  try {
    ...
    const pipelinePayload = (await pipelineResponse.json().catch(() => null)) as unknown
