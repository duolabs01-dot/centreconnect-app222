export function applicationStatusEmail({
  ...
  const statusLabels: Record<string, string> = {
    ...
    enrolled: 'is confirmed enrolled',
  ...
  const subject =
    newStatus === 'approved'
      ? `${centreName} has approved ${childName}'s application`
      : newStatus === 'enrolled'
        ? `${childName} is now enrolled at ${centreName}`
        : `Application update: ${childName} at ${centreName}`
  ...
  const html = `
    <div style="font-family: 'DM Sans', Arial, sans-serif; max-width: 560px; margin: 0 auto; padding: 24px;">
      <div style="background: #0891b2; padding: 20px 24px; border-radius: 12px 12px 0 0;">
        <p style="color: white; font-size: 12px; font-weight: 700; letter-spacing: 0.1em; margin: 0; text-transform: uppercase;">CentreConnect</p>
      </div>
      <div style="background: white; border: 1px solid #e2e8f0; border-top: none; padding: 24px; border-radius: 0 0 12px 12px;">
        ...
        <p style="color: #475569; font-size: 14px; margin: 0 0 20px;">Centre: <strong>${centreName}</strong></p>
