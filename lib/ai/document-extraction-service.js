...
function getTempUploadPath(file) {
    ...
    return (0, path_1.join)(process.cwd(), 'tmp', 'gemini-uploads', `${Date.now()}-${(0, crypto_1.randomUUID)()}.${extension}`);
}
...
async function extractStructuredDocumentWithGemini(input) {
    ...
    try {
        ...
        if (shouldFallbackToOcr) {
            const fallback = await extractWithTesseract(input);
            if (fallback.success) {
                return Object.assign(Object.assign({}, fallback), { message: 'AI extraction failed. We used a basic document scan instead.' });
            }
        }
        return {
            success: false,
