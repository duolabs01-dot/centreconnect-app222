type ConfettiStyle = 'child' | 'application' | 'approval'

const STYLE_COLORS: Record<ConfettiStyle, string[]> = {
  child: ['#22c55e', '#f59e0b', '#38bdf8', '#f472b6'],
  application: ['#2563eb', '#06b6d4', '#14b8a6', '#a78bfa'],
  approval: ['#10b981', '#34d399', '#60a5fa', '#fbbf24'],
}

function burst(colors: string[], pieces: number) {
...
export function triggerConfetti(style: ConfettiStyle) {
  burst(STYLE_COLORS[style], style === 'approval' ? 100 : 80)
}

export function triggerFirstTimeConfetti(key: string, style: ConfettiStyle) {
