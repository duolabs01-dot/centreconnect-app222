...
async function loadMergedEnv() {
    ...
    for (const file of filesToTry) {
        try {
            ...
            env = Object.assign(Object.assign({}, env), parseEnvFile(content));
        }
        catch ( /* ignore */_a) { /* ignore */ }
    }
    return Object.assign(Object.assign({}, env), process.env);
}
async function main() {
    ...
    try {
        ...
        const { data: importRes, error: importErr } = await admin
            ...
            .single();
        if (importErr)
            throw new Error(`Import record creation failed: ${importErr.message}`);
        ...
        const { data: childRes, error: childErr } = await userClient
            ...
            .single();
        if (childErr)
            throw new Error(`Child creation failed: ${childErr.message}`);
        childId = childRes.id;
        ...
        if (attendanceErr)
            throw new Error(`Attendance creation failed: ${attendanceErr.message}`);
