...
async function main() {
    ...
    if (!email)
        missing.push('UAT_PLATFORM_ADMIN_EMAIL');
    if (!password)
        missing.push('UAT_PLATFORM_ADMIN_PASSWORD');
    if (missing.length > 0) {
    ...
    if (!response.ok || !(payload === null || payload === void 0 ? void 0 : payload.access_token)) {
        const reason = (payload === null || payload === void 0 ? void 0 : payload.error_description) || (payload === null || payload === void 0 ? void 0 : payload.msg) || (payload === null || payload === void 0 ? void 0 : payload.error) || `HTTP ${response.status}`;
        console.error(`Failed to fetch platform admin token: ${reason}`);
        process.exit(1);
        return;
    }
    const token = String(payload.access_token);
    const expiresIn = Number((_a = payload.expires_in) !== null && _a !== void 0 ? _a : 0);
