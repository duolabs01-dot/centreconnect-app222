...
exports.useWebPush = useWebPush;
const react_1 = require("react");
const client_1 = require("@/lib/supabase/client"); // Assuming you have a Supabase client configured
...
function useWebPush(userId, ecdId) {
    const [isSubscribed, setIsSubscribed] = (0, react_1.useState)(false);
    const [subscriptionLoading, setSubscriptionLoading] = (0, react_1.useState)(false);
    const supabase = (0, client_1.createClient)();
    // VAPID Public Key from environment variables
    const VAPID_PUBLIC_KEY = process.env.NEXT_PUBLIC_VAPID_PUBLIC_KEY;
    const registerServiceWorker = (0, react_1.useCallback)(async () => {
        ...
        try {
            ...
            if (subscription) {
                ...
                if (edgeFunctionError) {
                    ...
                    sonner_1.toast.error('Failed to enable notifications.');
                    setIsSubscribed(false); // Mark as not subscribed if server save fails
                }
                else {
                    sonner_1.toast.success('Notifications enabled!');
                }
            }
        }
        catch (error) {
            console.error('Failed to subscribe to push notifications:', error);
            sonner_1.toast.error('Failed to enable notifications. Please ensure you allow notifications in your browser settings.');
    ...
    const unsubscribe = (0, react_1.useCallback)(async () => {
        ...
        try {
            ...
            if (subscription) {
                ...
                sonner_1.toast.info('Notifications disabled.');
            }
        }
        catch (error) {
            console.error('Failed to unsubscribe from push notifications:', error);
            sonner_1.toast.error('Failed to disable notifications.');
        }
        finally {
            setSubscriptionLoading(false);
        }
    }, []);
    const requestPermissionAndSubscribe = (0, react_1.useCallback)(async () => {
        if (!('Notification' in window)) {
            sonner_1.toast.error('This browser does not support notifications.');
            return;
        }
        if (Notification.permission === 'denied') {
            sonner_1.toast.error('Notification permission denied. Please enable it in your browser settings.');
            return;
        }
        if (Notification.permission === 'default') {
            ...
            if (permission === 'granted') {
                ...
                sonner_1.toast.error('Notification permission not granted.');
            }
        }
