...
exports.evaluateApplicationDocumentChecklist = evaluateApplicationDocumentChecklist;
exports.toApplicationDocumentLabel = toApplicationDocumentLabel;
exports.toApplicationDocumentLabels = toApplicationDocumentLabels;
...
function matchesAlias(docType, alias) {
    return docType === alias || docType.includes(alias);
}
function hasRequirement(docTypes, aliases) {
    return docTypes.some((docType) => aliases.some((alias) => matchesAlias(docType, alias)));
}
function evaluateApplicationDocumentChecklist(docTypes) {
    const normalized = docTypes.map(normalizeDocType).filter(Boolean);
    const completed = REQUIRED_APPLICATION_DOCUMENTS.filter((document) => hasRequirement(normalized, document.aliases));
    const missing = REQUIRED_APPLICATION_DOCUMENTS.filter((document) => !hasRequirement(normalized, document.aliases));
    return {
...
function toApplicationDocumentLabel(code) {
    const found = REQUIRED_APPLICATION_DOCUMENTS.find((document) => document.code === code);
    if (found)
        return found.label;
    return code.replaceAll('_', ' ');
}
function toApplicationDocumentLabels(codes) {
