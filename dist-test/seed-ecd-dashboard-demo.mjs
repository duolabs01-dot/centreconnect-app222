...
async function main() {
    ...
    if (!centre) {
        ...
        if (fallbackErr || !fallbackCentre) {
            throw fallbackErr !== null && fallbackErr !== void 0 ? fallbackErr : new Error('No active ECD centre found for demo seeding');
        }
    ...
    const profileUpsert = await supabase.from('user_profiles').upsert({
        id: authUser.id,
        role: 'ecd_admin',
        full_name: (_b = (_a = authUser.user_metadata) === null || _a === void 0 ? void 0 : _a.full_name) !== null && _b !== void 0 ? _b : 'ECD Admin Demo',
        phone: (_c = authUser.phone) !== null && _c !== void 0 ? _c : null,
    }, { onConflict: 'id' });
    if (profileUpsert.error)
        throw profileUpsert.error;
    const parentUpsert = await supabase.from('parents').upsert({
        id: authUser.id,
        alt_phone: (_d = authUser.phone) !== null && _d !== void 0 ? _d : '+27 82 000 0000',
        city: 'Johannesburg',
        province: 'Gauteng',
    }, { onConflict: 'id' });
    if (parentUpsert.error)
        throw parentUpsert.error;
    ...
    const applicationRows = childRows.map((child, idx) => {
        var _a, _b;
        const submittedAt = isoHoursAgo((_a = submittedHoursAgo[idx]) !== null && _a !== void 0 ? _a : 24);
        const status = (_b = statusPlan[idx]) !== null && _b !== void 0 ? _b : 'submitted';
    ...
    if (pickupUpsert.error)
        throw pickupUpsert.error;
