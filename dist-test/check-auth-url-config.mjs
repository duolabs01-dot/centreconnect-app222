...
function hasBadHost(value) {
    const lower = String(value !== null && value !== void 0 ? value : '').toLowerCase();
...
const env = Object.assign(Object.assign({}, mergedFileEnv), process.env);
const issues = [];
const warnings = [];
const appUrlRaw = (_a = env.NEXT_PUBLIC_APP_URL) !== null && _a !== void 0 ? _a : '';
const emailAppUrlRaw = (_b = env.NEXT_PUBLIC_EMAIL_APP_URL) !== null && _b !== void 0 ? _b : '';
const rootDomainRaw = (_c = env.NEXT_PUBLIC_ROOT_DOMAIN) !== null && _c !== void 0 ? _c : '';
const siteUrlRaw = (_g = (_f = (_e = (_d = env.SUPABASE_SITE_URL) !== null && _d !== void 0 ? _d : env.GOTRUE_SITE_URL) !== null && _e !== void 0 ? _e : env.SITE_URL) !== null && _f !== void 0 ? _f : env.AUTH_SITE_URL) !== null && _g !== void 0 ? _g : '';
...
if (appUrl && emailAppUrl && appUrl.hostname !== emailAppUrl.hostname) {
    issues.push(`NEXT_PUBLIC_APP_URL (${appUrl.hostname}) and NEXT_PUBLIC_EMAIL_APP_URL (${emailAppUrl.hostname}) must match`);
}
if (appUrl && siteUrl && appUrl.hostname !== siteUrl.hostname) {
    issues.push(`APP URL (${appUrl.hostname}) and Supabase SITE_URL (${siteUrl.hostname}) should match to avoid bad redirect links`);
}
if (issues.length > 0) {
