...
const mockCentres = [
  ...
  },
  {
    ...
    city: 'Johannesburg',
    suburb: 'Bramley',
    cover_image_url: 'https://images.unsplash.com/photo-1507878866276-a947ef722fee?auto=format&fit=crop&w=1600&q=80',
...
export default function ParentDiscoverPage() {
  ...
  const filteredCentres = useMemo(() => {
    ...
    return mockCentres.filter((centre) => centre.name.toLowerCase().includes(needle) || centre.suburb?.toLowerCase().includes(needle))
  }, [query])

  // TanStack Query ready structure (replace mock data when ready):
  // const { data: centres = [], isLoading } = useQuery('parent-centres', fetchParentCentres)
        ...
        {filteredCentres.length === 0 ? (
          <section className="flex flex-col items-center justify-center gap-4 rounded-3xl border border-white/10 bg-white/5 px-6 py-10 text-center text-slate-200 shadow-[0_30px_60px_rgba(15,23,42,0.35)]">
            <svg
              ...
              viewBox="0 0 160 160"
              className="h-32 w-32 text-amber-300"
            >
              <circle cx="80" cy="80" r="70" fill="currentColor" opacity="0.2" />
              <path
                d="M40 100c0-22 18-40 40-40s40 18 40 40"
                stroke="currentColor"
                strokeWidth="6"
                fill="none"
                strokeLinecap="round"
              />
              <path
                d="M60 72c4-8 12-12 20-12s16 4 20 12"
                stroke="currentColor"
                strokeWidth="6"
                fill="none"
                strokeLinecap="round"
