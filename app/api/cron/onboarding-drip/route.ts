...
export async function POST(request: Request) {
  ...
  for (const row of subscriptionRows ?? []) {
    if (!tierMap[row.ecd_id]) tierMap[row.ecd_id] = row.tier
  }
  ...
  for (const centre of centres) {
      ...
      if (!sentMap.has(key)) {
        try {
          ...
          const result = await deliverTransactionalEmail({
            to: ownerEmail,
            subject: `${centre.name} \u2014 pick up where you left off`,
            html,
          })
    ...
    if (centre.onboarding_complete && centre.onboarding_completed_at) {
      ...
      if (completionAgeDays >= 14 && completionAgeDays < 18) {
        ...
        if (!sentMap.has(key)) {
          try {
            const tier = (tierMap[centre.id] ?? 'basic') as 'basic' | 'standard' | 'premium'
            const slug = centre.slug?.trim() ?? ''
            ...
            const result = await deliverTransactionalEmail({
              to: ownerEmail,
              subject: `2 weeks in \u2014 here\u2019s what\u2019s working for ${centre.name}`,
              html,
            })
            await upsertNotificationLog(admin, {
              centreId: centre.id,
