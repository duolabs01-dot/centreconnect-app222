...
if (CENTRE_BLUEPRINTS.length !== 50) {
  throw new Error(
    `Expected 50 centre blueprints, received ${CENTRE_BLUEPRINTS.length}`
  )
}
...
async function main() {
  ...
  const insertedOrUpdated = data?.length ?? 0
  console.log(
    `Seed complete. Upserted ${insertedOrUpdated} centre records (input: ${centres.length}).`
  )
}
