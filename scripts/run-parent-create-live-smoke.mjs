  ...
  try {
    if (isDryRun) {
      ...
      if (userResult.error || !userResult.data?.user?.id) {
        throw new Error(userResult.error?.message || 'Failed to create auth user')
      }
      ...
      if (signIn.error || !signIn.data?.user?.id) {
        throw new Error(signIn.error?.message || 'Failed to sign in as test parent')
      }
      ...
      if (childInsert.error || !childInsert.data?.id) {
        throw new Error(childInsert.error?.message || 'Failed to create child profile')
      }
      ...
      if (emergencyInsert.error || !emergencyInsert.data?.id) {
        throw new Error(emergencyInsert.error?.message || 'Failed to create emergency contact')
      }
    ...
    if (report.checks.createParentAuthUser === 'pending') report.checks.createParentAuthUser = 'failed'
    if (report.checks.parentCanCreateChild === 'pending') report.checks.parentCanCreateChild = 'failed'
    ...
    if (isDryRun) {
      ...
      if (emergencyId) {
        const cleanupEmergency = await admin
          ...
          .delete()
          .eq('id', emergencyId)
        if (cleanupEmergency.error) cleanupErrors.push(cleanupEmergency.error.message)
      }

      if (childId) {
        const cleanupChild = await admin.from('children').delete().eq('id', childId)
        if (cleanupChild.error) cleanupErrors.push(cleanupChild.error.message)
      }

      const deleteUser = await admin.auth.admin.deleteUser(userId)
      if (deleteUser.error) cleanupErrors.push(deleteUser.error.message)
